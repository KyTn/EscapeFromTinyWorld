...
public class RubikController : MonoBehaviour {
    ...
    public static RubikController instance;

    public RubikCube RubikCubePrefab;

    public RubikCube[,,] cubes;
    ...
    */
    ...
    void ConstructRubikWorld()
    {
        ...
        for (int x = 0; x < 3; x++)
        {
            for (int y = 0; y < 3; y++)
            {
                for (int z = 0; z < 3; z++)
                {
                    r= (RubikCube) Instantiate(RubikCubePrefab, new Vector3(x, y, z), Quaternion.identity);
                    r.name = "RC(" + x + "," + y + "," + z + ")";
    ...
    public void RotateToAxis(RotationAxis rotAxis)
    {
        ...
        {
            if (AxisSelectorManager.instance.playerOnCube) return;
        }
            ...
            for (int y = 0; y < 3; y++)
            {
                ...
                for(int i = 0; i < hits.Length; i++)
                {
                    if (hits[i].collider.gameObject.CompareTag("RubikCube"))
                    {
                        ...
                        cubes.SetValue(hits[i].collider.gameObject.GetComponent<RubikCube>(), x, y, z);
